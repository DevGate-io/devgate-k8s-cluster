...
ARG PNPM_VERSION
# corepack prepare падает в изолированной сети (registry.npmjs.org недоступен в buildkit),
# поэтому ставим pnpm через npm — тот же реестр, но без бага corepack с fetch.
